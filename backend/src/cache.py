...
def monitoring_results(monitoring_ocel: str, constraint_graphs: str) -> str:
    # Object types are hashed twice because apparently, there is a maximal file name length.
    return f"monitoring-{hash_path(monitoring_ocel)}-{hash('_'.join([hash(constraint_graphs)]))}"

def action_engine_results(monitoring_ocel: str, constraint_patterns: str, action_graph: str, action_conflicts: str) -> str:
    # Object types are hashed twice because apparently, there is a maximal file name length.
    return f"action-engine-{hash_path(monitoring_ocel)}-{hash('_'.join([hash(constraint_patterns)]))}"

def build_aim_results(ocel: str) -> str:
    # Object types are hashed twice because apparently, there is a maximal file name length.
    return f"build-aim-{hash_path(ocel)}"

def impact_analysis_results(ocel: str, change: str, performance_analysis_window: str) -> str:
    # Object types are hashed twice because apparently, there is a maximal file name length.
    return f"impact-analysis-{hash_path(ocel)}-{hash(change)}-{hash(performance_analysis_window)}"
